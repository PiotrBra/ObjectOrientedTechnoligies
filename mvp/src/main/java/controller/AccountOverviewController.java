...
public class AccountOverviewController {
	...
	private void handleAddAction(ActionEvent event) {
		// Utwórz nowy, pusty obiekt Transaction
		Transaction newTransaction = Transaction.newTransaction();

		// Otwórz okno dialogowe do edycji transakcji
		boolean okClicked = appController.showTransactionEditDialog(newTransaction);

		// Jeśli użytkownik zatwierdzi dialog, dodaj nową transakcję do konta
		if (okClicked) {
			data.getTransactions().add(newTransaction);
		}
	}
